function out = calc_heading(t,samp,phins)
    ...
    out.mag_ned = mag;
    figure(11);plot(out.mag_ned(1,:),out.mag_ned(2,:),'.');grid on;axis equal;xlabel('x (gaus)');ylabel('y (gaus)');title('Cor Mag - Local Level Plane');
    pt = taxis(phins.t); 
    
    figure(12);plot(taxis(phins.t),out.att_error,'.');grid on;xlabel(tlabel(phins.t));ylabel('degrees');title('Att Error');xlim([pt(1),pt(end)]);ylim([-50,50]);legend(sprintf('roll=%f', rms_err(1)),sprintf('pitch=%f', rms_err(2)),sprintf('heading=%f', rms_err(3)));
    figure(13);plot(taxis(phins.t),phins.att,'.',taxis(phins.t),out.att*180/pi,'.');grid on;legend('px','py','pz','x','y','z');xlabel(tlabel(phins.t));title('Attitude');xlim([pt(1),pt(end)]);ylim([-200,200]);
